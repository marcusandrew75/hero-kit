...
// ─── Landing page ─────────────────────────────────────────────────────────────
// Marketing front-door for HeroKit, reachable at ?landing (the tool itself
// stays at the bare URL). Same Braun/Teenage Engineering design language as
// the tool: light surfaces, hatch marks, sequential numbering, mono spec
// readouts, one orange accent. Headings set in Space Grotesk.
//
...
//   gallery-01.jpg … gallery-08.jpg   exported HeroKit outputs (~1600×1000)
//   ui-sidebar.jpg                    tall screenshot of the control sidebar
//   ui-mask.jpg                       Effect Mask being painted on the canvas
//   ui-preview.jpg                    Preview in Context overlay
...
const GROTESK: React.CSSProperties = { fontFamily: '"Space Grotesk", sans-serif', letterSpacing: '-0.02em' };

// ─── Micro-components ─────────────────────────────────────────────────────────

/** Hatch-mark cluster — the ///// motif from the tool's panel headers. */
const Hatch: React.FC<{ count?: number }> = ({ count = 5 }) => (
...
const SectionTag: React.FC<{ number: string; label: string }> = ({ number, label }) => (
...
const SmartImg: React.FC<{ src: string; alt: string; ratio: string; className?: string; onExpand?: (src: string) => void }> =
...
const Led: React.FC<{ delay?: number }> = ({ delay = 0 }) => (
...
const Avatar: React.FC<{ src: string; alt: string; initial: string; size?: number }> =
...
const LandingPage: React.FC<{ onOpen: () => void }> = ({ onOpen }) => {
  ...
  // Scroll-reveal: any element with .landing-reveal fades/rises in once.
  useEffect(() => {
